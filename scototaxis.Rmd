...
```{r}
dur_sums <- rowSums(raw[5:6]) 
raw <- cbind(raw, dur_sums)

dur_sums <- rowSums(form[5:6]) 
form <- cbind(form, dur_sums)

dur_sums <- rowSums(lat[5:6]) 
lat <- cbind(lat, dur_sums)

dur_prop <- lapply(raw[5:6], function(x){x/dur_sums}) 
raw <- cbind(raw, dur_prop) #creates two columns with same name; use name function below to specify column name
names(raw)[16] <- "dur_prop.W"
names(raw)[17] <- "dur_prop.B"

dur_prop <- lapply(form[5:6], function(x){x/dur_sums}) 
form <- cbind(form, dur_prop) #creates two columns with same name; use name function below to specify column name
names(form)[16] <- "dur_prop.W"
names(form)[17] <- "dur_prop.B"

dur_prop <- lapply(lat[5:6], function(x){x/dur_sums}) 
lat <- cbind(lat, dur_prop) #creates two columns with same name; use name function below to specify column name
...
avg.dur_prop.W <-aggregate(dur_prop.W~X.U.FEFF.Name, raw, mean)

avg.dur_prop.B <-aggregate(dur_prop.B~X.U.FEFF.Name, raw, mean)

raw2 <- cbind(avg.dur_prop.W, avg.dur_prop.B)
raw2 <- raw2[-c(3)]
...
```{r}
wilcox.test(dur_prop.W~spp, data = raw2, paired=FALSE, mu=0.5)
wilcox.test(dur_prop.B~spp, data = raw2, paired=FALSE, mu=0.5)

```

<font color= "deeporchid4"> ***RESULTS:***</font> got non-significant results for both tests...in fact, got the same p-value (0.06061), so I'm not sure if that's a result of having so few sailfins or if I did the test wrong. At the moment, it does not seem that the species differ in their 